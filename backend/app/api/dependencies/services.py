...
def get_prompt_workspace_service() -> PromptWorkspaceService:
    database = get_database()
    company_repository = CompanyRepository(database)
    integration_repository = LLMIntegrationRepository(database, company_repository)
    prompt_run_repository = PromptRunRepository(database)
    return PromptWorkspaceService(integration_repository, prompt_run_repository)
